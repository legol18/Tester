...
!         endif
 ...
 END MODULE information
    ...
    use DVODE_F90_M
      ...
      IMPLICIT NONE
      integer:: i,ii,j,kij,ipar1,ipar2,inic,iter,ieff,iprob,indx1,indx2,kcount,irepchk
...
!      write(1,*) "digraph{"
       do i=1,nptch
        beta(i)=0.5d0!0.01d0+(1.d0-0.01d0)*dfloat(i-1)/dfloat(NEQ)  ! AREA PROXIES for Waxman
       enddo
...
!!!!!! Autocorrelation calculation     
  ...
  sd2geary=dsqrt(m2geary-fmgeary**2)/dsqrt(dfloat(ninic-kcount))
    write(11,24)dfloat(nptch),fmmoran,sd2moran,fmgeary,sd2geary,dfloat(ninic-kcount)
  print*,kcount
   if(kcount>35.and.irepchk<5) then
     irepchk=irepchk+1
     goto 11
   endif  
      24  Format(52F17.10)
