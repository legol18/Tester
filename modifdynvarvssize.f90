 MODULE information
   ...
   integer, parameter:: niter=50000,ntrans=50000, net_Tscl=5000, id=2, ninic=300
   real(kind=8), parameter:: d_max=14000.d0, a_max=1000.d0, h=0.1d0 
   real(kind=8):: par(20000), xL, xbta
...
!         endif
 ...
 END MODULE information
    ...
    use DVODE_F90_M
      ...
      real(kind=8):: p_waxmn, p_mc, T, TOUT, xprob
      real(kind=8):: xlbd, mov_par
      real(kind=8),allocatable:: Y(:)      !! dynamical variables
...
!      write(1,*) "digraph{"
       do i=1,nptch
       beta(i)=xbta  ! AREA PROXIES for Waxman
       enddo
...
!    write(11,24) dfloat(nptch),ymean,y2mean,Ysd/ymean
  ...
  if(kcount>40.and.irepchk<5) then
     irepchk=irepchk+1
     goto 11
   endif
    ...
    write(11,24) dfloat(nptch), yinicon, yiniconsd, dfloat(kcount)
